...
public class CandidatePreProcess
{
			...
			while(carflag<6)//頭尾排班第一輪夜班 第二輪汐止土城 第三輪特殊車輛 第四輪小車 第五輪一般車選
			 {
				...
				{ 				
					for(int l=0;l<40;l++)//時間總區間數
					{ 
						for(int k=0;k<DrTable.get(j).getCar(l).size();k++)
						{  
							int[] earaarry=new int[10];//紀錄主要支援地區
							int[] earaarry1=new int[10];//輔助支援地區								
							int arraycount1=-1,arraycount=-1;//紀錄主要跟輔助地區的數量	
							int searchrang=0,specialcarsearchrang=1,specialcar=0;///gh marked 2012/1/11 增加搜尋範圍參數							
							...
							else if(carflag==1&&DrTable.get(j).getCar(l).get(k).station.equals("土城"))//土城排頭尾班
							{
							...
							proupdate.updatedatabase(5, recentPercent,variable.date,variable.time); //更新進度表
							}
				 }
			 }  
				carflag++;
	...
	{	// Node:司機資訊  carindex:目前所搜尋地區  searchrang:搜尋範圍  specialcar:特殊班次flag如果為1將修改資料庫時間	
		...
		int startimeinx=((int)(starttime-21600)/1800)-searchrang;//頭班時間往前一格
		if(startimeinx<0)
			startimeinx=0;
		
		 for(int l=0;l<=startimeinx+searchrang;l++)//搜尋符合時間內的車子
		 { 
	...
	{		
		...
		XY[1] = Node.Y;		
		address[0] = Node.Address;
		int startimeinx=(int)(starttime-21600)/1800;//頭班搜尋起始區間	
		int latertime=starttime+900;//允許頭班晚15分鐘出班	
			 ...
			 for(int k=0;k<ReqTable.get(carindex).getreq(l).size();k++)
				 ...
				 {  
					...
					//回傳小於等於-2代表google查詢有錯立即終止程式
					 ...
					 if(carindex==1&&Node.station.equals("中和")&&(oncartime+ilfReturn)>(1200+variable.map_Revise_Traveltime))
						 continue;//如果中和支援新店旅行時間加尖峰時刻超過20分不排入					 
					 if(ilfReturn > -1)
					 {  					
						 if(((Node.StartTime + ilfReturn+oncartime)-ReqTable.get(carindex).getreq(l).get(k).OriginTime)<=variable.map_Revise_Traveltime)//判斷來不來的及接到預約者	
							 {	
		...
		if(Target != null)
		{
			if(Node.station.equals("中和"))
				if(Target.Originarea.equals("新北市三峽區")||Target.Originarea.equals("新北市鶯歌區")||Target.Destinationarea.equals("新北市三峽區")||Target.Destinationarea.equals("新北市鶯歌區"))
				{
					Node.EndTime=Node.StartTime+32400-1800;//有支援三峽鶯歌的中和車輛提早半小時下班
					writetime=1;
	...
	//排尾班
		...
		private boolean OrderEnd(DriverTable Node,int searchrang,int carindex,int specialcar) throws Exception
		{
			...
			int minendtime=endtime-900;//允許延遲時間	
			...
			int timeinx=(int)(endtime-21600)/1800;	//晚班起始區間
						...
						if(Tailreq.get(carindex).getreq(l).get(k).OriginTime>=57000&&Tailreq.get(carindex).getreq(l).get(k).OriginTime<=endtime&&(Tailreq.get(carindex).getreq(l).get(k).OriginTime>=minendtime)&&Tailreq.get(carindex).getreq(l).get(k).Status == 2
						 ...
						 {	
							 ...
							 if(ilfReturn<=-2)
							{
							 ...
							 //預約者下車時間到回廠落在下午16:45~18:30尖峰時段再加上30分延遲
							 ...
							 if(carindex==1&&Node.station.equals("中和")&&(oncartime+ilfReturn)>(2100+variable.map_Revise_Traveltime))
								 continue;//如果中和支援新店旅行時間加尖峰時刻超過20分不排入 如果提前15分放寬
