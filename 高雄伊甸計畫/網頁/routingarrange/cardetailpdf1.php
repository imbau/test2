<?php
	...
	fclose( $File );	
	$pos = strrpos($Output, "<p class=MsoNormal align=center style='text-align:center;line-height:20.0pt'>");	
	$head=substr($Output,0,$pos);		
	$pos = strrpos($Output, "</div>");
	$foot=substr($Output,$pos);
	
	$pos = strrpos($Output, "<p class=MsoNormal align=center style='text-align:center;line-height:20.0pt'>");	
	$pos1 = strrpos($Output, "</body>");
	$lenth=$pos1-$pos;
	$Output1=substr($Output,$pos,$lenth);	
	$table="";
	$index=1;
	$caridrs = mysql_query("SELECT * FROM arrangedtable WHERE date = '".$arrangedate."'  AND arrangetime = '".$arrangetime."' ORDER BY no");
	...
	do
	{
	    if($caridrsrow['carid']!="")
		{
			$table=$table.writedata($Output1,$arrangedate,$arrangetime,$caridrsrow['carid']);
		   if($index<$num_rows )
				$table=$table.newpage();			
			$index++;
		}
	...
	function newpage()
	{
	   $newpage="
					<b>
							<span lang=EN-US style='font-size:16.0pt;font-family:標楷體;mso-bidi-font-family:新細明體;
									   color:black;mso-ansi-language:EN-US;mso-fareast-language:ZH-TW;mso-bidi-language:
									  AR-SA'><br clear=all style='mso-special-character:line-break;page-break-before:always'>
					       </span>
					</b>";
		return  $newpage;
	}
	...
	function writedata($Output,$arrangedate,$arrangetime,$carid)
	{
	...
	$Output = str_replace( "(carid)", mb_convert_encoding($carid."  (".$carclassrow['呼號'].")",'Big5','auto'), $Output );
