 ...
 $( "#dialog-form1" ).dialog
					(
							...
							"資料送出": function()
							{
                               ...
                               if($("input#account1").val() == "")
                              ...
                              else if($("input#startarea1").val() == ""|| switchareaindex($("input#startarea1").val())==-1)
									{
                                        alert("請輸入正確上車區域!!");
                                ...
                                else if($("input#endarea1").val() == ""|| switchareaindex($("input#endarea1").val())==-1)
									{
                                        alert("請輸入正確下車區域!!");
      ...
      function controltable()
	   ...
	   function reqtable()
       {
		window.open ("reqtable.php?date="+arrangedate+"&time="+arrangetime+"&mode=0","newwindow3", "location=1, status=1, scrollbars=1, width=1200, height=750");
	  }
       ...
       function Candidate()
		...
		{	
		 ...
		 function open_win(reqnum,Reason) {			 				
          ...
          function open_win_CandidatePreProcess() {
		            $.get("<?php  echo $linkurlport; ?>/WebRoutingArranger/headtailteam.view", {arrangetime: arrangetime, arrangedate: arrangedate, mode: 2},
					function(response) 
					{ 		
						    alert(checkstatus(response,newwin));						
					}
					);	
					newwin=window.open('<?  echo $linkurlport;?>'+"/routingarrange/redirect.php?date="+arrangedate+"&time="+arrangetime+"&mode=5","newwindows","height=100, width=500");	
					   }
  ...
  <iframe id="preview-frame" src="menu.php" name="preview-frame" frameborder="0"  noresize="noresize" style="width: 1150px;  height:225px; position: relative; z-index: 500;">
    ...
    <div class="well1">	   
	 ...
	 return $message;
      ...
      $endhour = 24;
      $tolertime =0;
      $rs1 = mysql_query("SELECT A1.*,A2 .*	FROM arrangedtable A1, availablecars A2 WHERE A1.carid = A2.車號  AND  A2. date= '".$arrangedate."' AND A2.time = '".$arrangetime."' AND  A1. date = '" .$arrangedate."' AND A1.arrangetime = '".$arrangetime."' AND A2.站名='".$area."' GROUP BY A1.`carid` ORDER BY A1.worktime ASC");
	  ...
	  $progressrow = mysql_fetch_array($progress);
	   ?>		
		<?php    
		echo "<div style=\"width:1080px;height: 600px;\">";
              ...
              <tr>
				...
				<td><label for="date1">排班時間</label><select name="date1" id="date1">
                  <option value="-1">
                   ...
                   <?
				   $log= mysql_query("SELECT * FROM arrange_log ORDER BY no DESC LIMIT 0, 15");
